...
namespace Application.UseCases.Interface.Delete
{
    public class DeleteInterface(IInterfaceRepository interfaceRepository) : IUseCase<UseCaseResult, Request>
    {
        public async Task<UseCaseResult> Handle(Request request, CancellationToken cancellationToken)
        {
            var result = await interfaceRepository.DeleteAsync(request.Interface, cancellationToken);
