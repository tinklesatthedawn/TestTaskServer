...
namespace Application.UseCases.Register.Create
{
    public class CreateRegister(IRegisterRepository registerRepository) : IUseCase<UseCaseResult, Request>
    {
        public async Task<UseCaseResult> Handle(Request request, CancellationToken cancellationToken)
        {
            var result = await registerRepository.AddAsync(request.Register, cancellationToken);
