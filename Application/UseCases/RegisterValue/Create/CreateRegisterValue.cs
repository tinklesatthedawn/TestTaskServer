...
namespace Application.UseCases.RegisterValue.Create
{
    public class CreateRegisterValue(IRegisterValueRepository registerValueRepository) : IUseCase<UseCaseResult, Request>
    {
        public async Task<UseCaseResult> Handle(Request request, CancellationToken cancellationToken)
        {
            var result = await registerValueRepository.AddAsync(request.RegisterValue, cancellationToken);
