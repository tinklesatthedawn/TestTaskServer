...
namespace Application.UseCases.RegisterValue.Update
{
    public class UpdateRegisterValue(IRegisterValueRepository registerValueRepository) : IUseCase<UseCaseResult, Request>
    {
        public async Task<UseCaseResult> Handle(Request request, CancellationToken cancellationToken)
        {
            var result = await registerValueRepository.UpdateAsync(request.RegisterValue, cancellationToken);
